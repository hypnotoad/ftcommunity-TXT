...
class StatusBar(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(QRect(QPoint(0, 0), self.size()),
              Qt.AlignmentFlag.AlignRight, QLocale().toString(QTime.currentTime(), QLocale.FormatType.ShortFormat))
...
class TouchTopWidget(QWidget):
    ...
    def __init__(self, parent=None):
        super(TouchTopWidget, self).__init__(parent)
...
class Icon(QPixmap):
    def __init__(self, name, parent=None):
        super(Icon, self).__init__(os.path.join(BASE, "media", name), parent)
...
class AppItem(BaseItem):
    def __init__(self, name):
        super(AppItem, self).__init__(name)
...
class FolderItem(BaseItem):
    def __init__(self, name):
        super(FolderItem, self).__init__(name)
...
class FolderUpItem(FolderItem):
    ...
    def __init__(self, name, apps):
        super(FolderUpItem, self).__init__(name)
...
class AppList(list):
    """An app list contains a list of icons an folders and methods to work on them
    """
...
class Launcher(TouchApplication):
    ...
    def category_setup(self):
        ...
        if locale_str is not None:
            self.locale = QLocale(locale_str)
        else:
    ...
    def on_stop_app(self):
        if self.app_is_running():
            ...
            while self.app_process.poll() is None:
                pass
